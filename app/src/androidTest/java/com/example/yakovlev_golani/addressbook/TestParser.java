...
public class TestParser extends AndroidTestCase {
    ...
    public void testParser(){
        ...
        List<Result> resultList = randomUsers.getResults();
        Assert.assertEquals(3, resultList.size());

        User user = resultList.get(0).getUser();

        Name name = user.getName();
        Assert.assertEquals("mrs", name.getTitle());
        Assert.assertEquals("wanda", name.getFirst());
        Assert.assertEquals("riley", name.getLast());

        Assert.assertEquals("6176 railroad st", user.getLocation().getStreet());
        Assert.assertEquals("fayetteville", user.getLocation().getCity());

        // Need to add tests for all fields
